document.addEventListener('DOMContentLoaded', function() {
    ...
    function mostrarErrores(errores) {
        const alertaError = document.getElementById('errorAlertEstudiante');
        const listaErrores = document.getElementById('errorListEstudiante');
    ...
    const password = document.getElementById('estudiante_password');
    const confirmPassword = document.getElementById('estudiante_confirm_password');
    ...
    console.log('Configurando formulario de estudiante');
    ...
    if (form) {
        ...
        form.addEventListener('submit', async function(event) {
            ...
            document.getElementById('errorAlertEstudiante').style.display = 'none';
            const errores = [];

            // Validar RUT
            const rut = document.getElementById('estudiante_rut').value;
            const dv = document.getElementById('estudiante_div').value;
            if (!validarRut(rut, dv)) {
                errores.push('El RUT ingresado no es válido');
            }

            // Validar correo institucional
            const correo = document.getElementById('estudiante_correo').value;
            if (!validarCorreoInstitucional(correo)) {
                errores.push('El correo debe ser institucional (@gem.cl)');
            }

            // Validar fecha de nacimiento
            const fechaNacimiento = new Date(document.getElementById('estudiante_fecha_nacimiento').value);
            const hoy = new Date();
            const edad = hoy.getFullYear() - fechaNacimiento.getFullYear();
            
            if (edad < 10 || edad > 20) {
                errores.push('La edad del estudiante debe estar entre 10 y 20 años');
            }

            // Validar contraseñas
            const password = document.getElementById('estudiante_password').value;
            const confirmPassword = document.getElementById('estudiante_confirm_password').value;
            if (password !== confirmPassword) {
            ...
            // Validar curso
            const curso = document.getElementById('estudiante_curso');
            ...
            try {
                ...
                if (data.success) {
                    ...
                    mostrarMensajeExito('Estudiante creado exitosamente');
                    ...
                    // Mostrar errores
                    mostrarErrores(data.errors.__all__ || ['Error al crear usuario']);
                }
            } catch (error) {
